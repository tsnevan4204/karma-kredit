...
const BusinessDashboard = ({ walletAddress, myLoans, loading: parentLoading }) => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      console.log('[handleSubmit] Uploading to Pinata...');
      const cid = await uploadToPinata({ imageFile: form.image, description: form.description });
      console.log('[handleSubmit] Pinata CID:', cid);
      setLoanOptions([{ id: Date.now(), businessName: 'My Biz', amount: form.amount, description: form.description, creditScore: creditScore, image: form.image, interestRate: loanTerms?.interest_rate, ipfsCid: cid }]);
      setShowForm(false);
      setForm({ amount: '', description: '', image: null });
    } catch (err) {
      setError('Pinata upload failed: ' + (err.message || err));
      console.error('[handleSubmit] Pinata error:', err);
    } finally {
      setLoading(false);
    }
  };
